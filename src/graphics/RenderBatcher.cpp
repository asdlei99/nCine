...
void RenderBatcher::createBatches(const nctl::Array<RenderCommand *> &srcQueue, nctl::Array<RenderCommand *> &destQueue)
{
	...
	for (unsigned int i = 1; i < srcQueue.size(); i++)
	{
		...
		const bool prevIsBlendingEnabled = command->material().isBlendingEnabled();
		const GLenum prevSrcBlendingFactor = command->material().srcBlendingFactor();
		const GLenum prevDestBlendingFactor = command->material().destBlendingFactor();
